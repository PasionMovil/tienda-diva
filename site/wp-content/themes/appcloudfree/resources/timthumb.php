...
define ('ALLOW_EXTERNAL', 1);			// allow external website (override security precaution)


$abspath = str_replace('\\', '/', dirname(dirname(__FILE__))).'/';
$first_to_find = 'wp-content/themes/';
$theme_folder = substr($abspath, (strpos($abspath, $first_to_find) + strlen($first_to_find)));
$theme_folder = substr($theme_folder, 0, strpos($theme_folder, '/'));

define('ABSPATH__', str_replace($first_to_find.$theme_folder, '', $abspath));
define('WP_CONTENT_DIR', ABSPATH__ . 'wp-content');
define('SHORTINIT', true );
define('NOBLOGREDIRECT', true);
require_once( ABSPATH__. 'wp-includes/formatting.php' );
require_once( ABSPATH__. '/wp-load.php' );
$wp_upload_dir_data = wp_upload_dir();
$upload_path = $wp_upload_dir_data['basedir'];
//$upload_path = TK_RESOURCES_DIR;

define ('DIRECTORY_CACHE', $upload_path.'/cache');		// cache directory
...
function clean_source ($src) {
    ...
    $src = '/'.str_replace('//', '/', preg_replace ("/\.\.+\//", "", $src));
